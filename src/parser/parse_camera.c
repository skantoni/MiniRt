...
/*   Updated: 2026/07/07 16:30:59 by pavelino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    validate_range(t_camera *cam)
{
    if (cam->orientation.x < -1.0f || cam->orientation.x > 1.0f)
        return (print_error("Error: ", "Orientation x out of range [-1,1] ", NULL), 1);
    if (cam->orientation.y < -1.0f || cam->orientation.y > 1.0f)
        return (print_error("Error: ", "Orientation y out of range [-1,1] ", NULL), 1);
    if (cam->orientation.z < -1.0f || cam->orientation.z > 1.0f)
        return (print_error("Error: ", "Orientation z out of range [-1,1] ", NULL), 1);
